...
$(function () {
    ...
    //获取传来的问卷id，根据问卷id，查询具体问卷问题
    var answerId = getQueryString("answerId");
    if (answerId === "null" || null == answerId) {
        var issueId = getQueryString("issueId");
        queryQuestionnaireIssue(issueId);
    } else {
        queryQuestionnaireIssueAnswer(answerId);
    }

});

function queryQuestionnaireIssue(id) {
    var url = '/questionnaire/queryObject/' + id;
    commonAjaxGet(false, url, queryQuestionnaireByIdSuccess);
}

//根据id查询问卷id及 answer(回答)
function queryQuestionnaireIssueAnswer(id) {
    var url = '/questionnaireAnswer/queryQuestionnaireAnswer/' + id;
    commonAjaxGet(false, url, queryQuestionnaireByIdSuccess);
}
...
function queryQuestionnaireByIdSuccess(res) {
    console.log(res)
    if (res.code == '666') {
        res.data.questionList = JSON.parse(res.data.questionList);
        setQuestion(res);
    } else {
...
function setQuestion(result) {
    ...
    // this.questionList[1] =  result.data.questionList;
        ...
        for (var i = 0; i < questionList.length; i++) {
            ...
            switch (questionType) {
                ...
                case "2": //填空
                    ...
                    if (null != result.data.questionnaireAnswer) {
                        var answer= result.data.questionnaireAnswer.answer
                         answer = answer.substring(0, answer.length - 1).substring(1).replace(/\"/g, "").split(",");
                        question_div = $(question_div).append('<textarea class="answerContent">' + answer[i] + '</textarea>');        //添加输入文本框
                    } else {
    ...
    $(".container-fluidT").append('<div>\n' +
        '            <button class="btn btn-primary submitBtn" onclick="submitQuestionnaire()">退出浏览</button>\n' +
        '        </div>');
    if (result.data.questionnaireAnswer != null) {
        setQuestionAnswer(result.data.questionnaireAnswer.answer);
    }
}

//根据题目来铺结果
function setQuestionAnswer(answer) {
    //截取字符串首位及最后一位后转成数组
    console.log(answer)
    answer = answer.substring(0, answer.length - 1).substring(1).replace(/\"/g, "").split(",");
    var questionNum = $(".container-fluidT").find(".form-group").length;
    for (var i = 0; i < questionNum; i++) {
        ...
        switch (questionType) {
            case "0": //单选
            case "4": //量表
                $('input[name="a' + i + '"]').get(answer[i]).checked = true;
                break;
            case "1": //多选
                var str = answer[i].split("&")
                for (var j = 0; j < str.length; j++) {
                    $('input[name="a' + i + '"]').get(str[j]).checked = true;
                }
                break;
            case "2": //填空
                // $('.answerContent').val(answer[i]);
                break;
            case "3": //矩阵
                var chooseOpts = $(".container-fluidT").children(".form-group").eq(i).children(".tswjdc_table").children('tbody').find('tr').length;
                for (var j = 1; j < chooseOpts; j++) {
                    $('input[name="b' + (i + 1) + (j - 1) + '"]').get(answer[i]).checked = true;
                }
                break;
        }
    }
...
function onclickCheckChoose(questionnaireId, problemId) {
    ...
    console.log(checkedList.length == 0)
    console.log('进入if')
