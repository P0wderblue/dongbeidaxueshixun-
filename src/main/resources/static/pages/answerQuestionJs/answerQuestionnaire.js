...
$(function () {
    ...
    layer.load(2);
    questionnaireIssueId = getQueryString("questionnaireIssueId");
    queryQuestionnaireIssue(questionnaireIssueId);

});

function queryQuestionnaireIssue(questionnaireIssueId) {
    var url = '/questionnaire/queryQuestionnaireIssueId/' + questionnaireIssueId;
...
function submitQuestionnaire() {
    eORp = "l";
        ...
        var da = {
            'questionnaireId': questionnaireIssueId,
            'answer': JSON.stringify(answerListFinal),
            'answerTime': dateChange(timeStart),
            'endTime': dateChange(timeEnd),
            'responseTime':((dateChange(timeEnd)- dateChange(timeStart))/1000)+"ms",
            'submitIp': ipAddress
        };
        if (eORp == 'p') {
        ...
        var url = '/questionnaireAnswer/add';
        commonAjaxPost(false, url, da, addSuccess)
}
